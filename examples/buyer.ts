...
const {
  ...
  apiKey && entitySecret && walletAddress && seller && origin && ssh,
  'Configure Circle credentials, wallet, seller, origin and SSH_PUBLIC_KEY',
);
const client = initiateDeveloperControlledWalletsClient({
  apiKey,
  entitySecret,
});
const hex = (s: string | undefined): `0x${string}` => {
  ensure(s, 'CIRCLE_SIGNATURE_MISSING');
...
const scheme = new BatchEvmScheme({
  ...
  signTypedData: async (params) => {
    const data = JSON.stringify(
      {
        domain: { ...params.domain, chainId: params.domain.chainId.toString() },
...
export const buyer = new Buyer(
  ...
  scheme,
  '.state/buyer.sqlite',
  process.env.BUDGET_ATOMIC ?? '10000',
);
