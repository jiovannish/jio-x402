...
const state: any = existsSync(path)
  ...
  : {
      ...
      walletSetKey: randomUUID(),
      walletsKey: randomUUID(),
      approveKey: randomUUID(),
      depositKey: randomUUID(),
    };
function save() {
...
if (!apiKey.startsWith('TEST_API_KEY:'))
  throw new Error('Testnet key required');
...
async function wait(id: string) {
  for (let i = 0; i < 60; i++) {
    ...
    if (['FAILED', 'DENIED', 'CANCELLED'].includes(tx?.state ?? ''))
      throw new Error(`Transaction ${id}: ${tx?.state}`);
...
try {
  if (!state.registered) {
    ...
    try {
      const result = await registerEntitySecretCiphertext({
        apiKey,
        entitySecret: state.entitySecret,
        recoveryFileDownloadPath: '.state',
      });
      if (!result.data?.recoveryFile) throw new Error('Recovery file missing');
      writeFileSync('.state/circle-recovery.dat', result.data.recoveryFile, {
        mode: 0o600,
      });
  ...
  if (!state.walletSetId) {
    state.walletSetId = (
      await client.createWalletSet({
        name: 'Jio x402 Arc Testnet',
        idempotencyKey: state.walletSetKey,
      })
    ).data?.walletSet?.id;
    save();
  }
  ...
  if (!state.wallets) {
    state.wallets = (
      await client.createWallets({
        walletSetId: state.walletSetId,
        blockchains: ['ARC-TESTNET'],
        accountType: 'EOA',
        count: 2,
        idempotencyKey: state.walletsKey,
      })
    ).data?.wallets;
    save();
  }
  if (state.wallets?.length !== 2)
    throw new Error('Expected buyer and seller wallets');
  for (const wallet of state.wallets) {
    state.funded ??= {};
    if (!state.funded[wallet.id]) {
      const balances =
        (await client.getWalletTokenBalance({ id: wallet.id })).data
          ?.tokenBalances ?? [];
      const funded = balances.some(
        (b) =>
          b.token.tokenAddress?.toLowerCase() ===
            CHAIN_CONFIGS.arcTestnet.usdc.toLowerCase() &&
          Number(b.amount) >= 1,
      );
      if (!funded)
      ...
      state.funded[wallet.id] = true;
      save();
    }
  }
  console.log(
    'Buyer and seller EOA wallets created; testnet faucet requested.',
  );
  ...
  if (!state.approvalId) {
    state.approvalId = (
      await client.createContractExecutionTransaction({
        ...
        abiParameters: [chain.gatewayWallet, '1000000'],
        fee: { type: 'level', config: { feeLevel: 'MEDIUM' } },
        idempotencyKey: state.approveKey,
      })
    ).data?.id;
    save();
  }
  ...
  if (!state.depositId) {
    state.depositId = (
      await client.createContractExecutionTransaction({
        ...
        abiParameters: [chain.usdc, '1000000'],
        fee: { type: 'level', config: { feeLevel: 'MEDIUM' } },
        idempotencyKey: state.depositKey,
      })
    ).data?.id;
    save();
  }
  ...
  console.log({
    buyer: walletAddress,
    seller: state.wallets[1].address,
    deposit_id: state.depositId,
    gateway_balance: await response.json(),
  });
} catch (error: any) {
  // Circle/Axios exceptions may contain authorization headers; never dump them.
  console.error({
    setup_failed: true,
    http_status: error.response?.status ?? error.status,
    code: error.response?.data?.code ?? error.code,
    message: error.response ? 'Circle API rejected setup' : error.message,
  });
  process.exitCode = 1;
}
