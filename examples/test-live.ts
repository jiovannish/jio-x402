...
try {
  ...
  console.log({
    ...
    retry_same_order: true,
  });
  for (let i = 0; i < 40; i++) {
    const receipt = (await buyer.receipt(q.quote_id)) as any;
    writeFileSync(
      'docs/live-payment.json',
      JSON.stringify(
        { checked_at: new Date().toISOString(), ...receipt },
        null,
        2,
      ) + '\n',
    );
    if (
      receipt.compute_status === 'ready' ||
      receipt.compute_status === 'failed'
    ) {
      console.log({
        order: receipt.order_id,
        payment: receipt.payment_status,
        compute: receipt.compute_status,
        computer: receipt.computer_id,
        refund: receipt.refund_status,
      });
      ensure(receipt.compute_status === 'ready', 'COMPUTE_FAILED');
      break;
    }
