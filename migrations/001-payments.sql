...
CREATE TABLE IF NOT EXISTS jio_payments.orders (
  id uuid PRIMARY KEY,
  quote_id uuid NOT NULL UNIQUE REFERENCES jio_payments.quotes,
  owner text NOT NULL,
  ...
  payment_status text NOT NULL CHECK (
    payment_status IN ('submitting', 'unknown', 'rejected', 'accepted', 'settled')
  ),
  payment_response jsonb,
  provider_reference text,
  compute_status text NOT NULL DEFAULT 'waiting' CHECK (
    compute_status IN ('waiting', 'pending', 'ready', 'failed', 'expired', 'stopped')
  ),
  operation_id text,
  computer_id text,
  compute_response jsonb,
